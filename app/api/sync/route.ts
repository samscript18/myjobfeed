...
export async function GET() {
	...
	try {
		...
		const [arbeitJobs, museJobs, jobicyJobs, joobleJobs] =
			await Promise.all([
				fetchArbeitnow(),
				fetchTheMuse(),
				fetchJobicy(),
		...
		console.log(
			`[SYNC] Total fetched jobs: ${normalizedJobs.length}`,
		);
		...
		console.log(
			`[SYNC] Deduplicated jobs: ${uniqueJobs.length}`,
		);
		...
		console.log(
			`[SYNC] Valid jobs after filtering: ${cleanJobs.length}`,
		);
		...
		for (const job of cleanJobs) {
			...
			if (
				!categorySlug ||
				!categoryMap.has(categorySlug)
			) {
				toProcessAI.push({
					title: job.title,
					description: cleanDescription.slice(
						0,
						800,
					),
					job,
		...
		console.log(
			`[SYNC] Deleted ${deleted.deletedCount} old jobs`,
		);
