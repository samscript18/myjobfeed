...
const SelectTrigger = React.forwardRef<React.ElementRef<typeof SelectPrimitive.Trigger>, SelectTriggerProps>(({ className, children, isLoading, disabled, helperText, ...props }, ref) => (
	<div className="w-full">
		...
		>
			{children}
			<SelectPrimitive.Icon asChild>{isLoading ? <Loader2 className="h-4 w-4 animate-spin opacity-50" /> : <ChevronDown className="h-4 w-4 opacity-50" />}</SelectPrimitive.Icon>
		</SelectPrimitive.Trigger>
		{helperText && <p className="mt-1 text-xs font-medium text-destructive">{helperText}</p>}
	</div>
));
...
const SelectItem = React.forwardRef<React.ElementRef<typeof SelectPrimitive.Item>, React.ComponentPropsWithoutRef<typeof SelectPrimitive.Item>>(({ className, children, ...props }, ref) => (
	<SelectPrimitive.Item
		ref={ref}
		className={cn(
			"relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-8 pr-2 text-sm outline-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50 focus:bg-primary/90 focus:text-accent-foreground",
			className,
		)}
		{...props}
	>
